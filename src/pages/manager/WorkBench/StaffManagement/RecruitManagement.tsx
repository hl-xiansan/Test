...
const useListView = (url: string, tabKey: string, props: any, status = -1,invite_code:string | null = workerInfo.getInviteCode()): any => {
  ...
  const fetchtList = async (flag = false) => {
    ...
    try {
      const params: any = {page: page.current, size: 10, invite_code: invite_code}
        ...
        <FlatList
          ...
          renderItem={(item: any) => <View key={String(item.item.user_id)} style={styles.positionItem}>
            <StaffManagementItem
              ...
              data={item.item}
              onPress={() => {
                props.navigation.navigate('EmployeeInfo', {id: item.item.id})
              }}
            />
          </View>}
...
const StaffManagementFc = (props: any) => {
  ...
  const [list, fetchtList, render1] = useListView('/labor/staff/worker/invite/list/', NAV[0], props)  // 全部
  const [list2, fetchtList2, render2] = useListView('/labor/staff/worker/invite/list/', NAV[1], props, 0) //待处理
  const [list3, fetchtList3, render3] = useListView('/labor/staff/worker/invite/list/', NAV[2], props, 1) //在职
  const [list4, fetchtList4, render4] = useListView('/labor/staff/worker/invite/list/', NAV[3], props, 3) //离职
  const [list5, fetchtList5, render5] = useListView('/labor/staff/worker/invite/list/', NAV[4], props, 2) //黑名单
