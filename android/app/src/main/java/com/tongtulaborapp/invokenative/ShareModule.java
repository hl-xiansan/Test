package com.tongtulaborapp.invokenative;
                        ...
                        public void onResult(int resultCode, String memo, Bundle bundle) {
                            if (resultCode == OpenAuthTask.OK) {
                                ...
                                for (String key:bundle.keySet()){
                                    result.putString(key, bundle.get(key).toString());
                                    Log.e("todoremove","key="+key+"   value"+bundle.get(key).toString());
                                }
                                successCallback.invoke(0,result,"success");
                            } else {
                                WritableMap result = Arguments.createMap();
                                successCallback.invoke(1,"fail");
                            }
                        }
                    };
                    // 传递给支付宝应用的业务参数
                    final Map<String, String> bizParams = new HashMap<>();
                    bizParams.put("url", "https://authweb.alipay.com/auth?auth_type=PURE_OAUTH_SDK&app_id=2021001169677768&scope=auth_user&state=QUFhc29kZmFzZGY");
                    final String scheme = "ap2021001169677768";
                    // 唤起授权业务
                    final OpenAuthTask task = new OpenAuthTask(ma);
                    task.execute(
